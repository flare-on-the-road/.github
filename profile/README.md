...
flowchart LR
    user["사용자"]
    frontend["Next.js Frontend<br/>App Router<br/>Zustand Auth Store<br/>CCTV / Events / AI Lab UI"]
    proxy["Next.js API Proxy<br/>/api/[...path]"]
    backend["Flask Backend API<br/>Route / Service / Repository / Model<br/>JWT / OAuth / Admin / Board / Events"]
    db[("Database<br/>users / posts / comments / events")]
    r2[("Cloudflare R2<br/>CCTV snapshots<br/>presigned download URL")]
    worker["Python Worker<br/>CCTV capture pipeline<br/>ffmpeg / OpenCV fallback"]
    its["ITS CCTV Open API<br/>CCTV list / stream URL"]
...
flowchart TB
    ...
    routes["routes<br/>HTTP endpoint<br/>request parsing"]
    services["services<br/>business rules<br/>external API integration"]
    repositories["repositories<br/>query boundary"]
    models["models<br/>SQLAlchemy ORM"]
    database[("Database")]
    common["common<br/>errors / response / uploads / decorators"]
    config["config / extensions<br/>env / db / cors / migrate / swagger"]
...
```mermaid
flowchart TB
    pages["app<br/>route pages<br/>server/client composition"]
    templates["templates<br/>shared page layout"]
    organisms["organisms<br/>feature-level UI"]
    molecules["molecules<br/>composed UI blocks"]
    atoms["atoms<br/>primitive UI components"]
    services["services<br/>typed API clients"]
    stores["stores<br/>Zustand auth state"]
    hooks["hooks<br/>auth/admin guards"]
    types["types<br/>domain contracts"]
...
sequenceDiagram
    ...
    participant ITS as ITS CCTV API
    participant R2 as Cloudflare R2
    participant Vision as Vision FastAPI
    participant VLM as VLM Client
    participant Backend as Flask Backend

    Worker->>ITS: 대상 CCTV 스트림 URL 조회
    ITS-->>Worker: CCTV metadata / stream URL
    Worker->>Worker: ffmpeg 우선 캡처, 실패 시 OpenCV fallback
    Worker->>R2: snapshot 이미지 저장
    R2-->>Worker: snapshot key 반환
    Worker->>Vision: 캡처 이미지 추론 요청
    Vision-->>Worker: fire / smoke / carlight 탐지 결과
    Worker->>Backend: POST /api/events
    Backend-->>Worker: event id
    opt 2차 판단 필요
        Worker->>VLM: 위험 후보 이미지 판단 요청
        VLM-->>Worker: is_fire / reason
        Worker->>Backend: PATCH /api/events/{id}
    end
```
...
erDiagram
    EVENT {
        ...
        int risk_score
        boolean risk_candidate
        boolean is_fire
        text vlm_reason
        json detected_classes
        string snapshot_key
        datetime created_at
    }
```
